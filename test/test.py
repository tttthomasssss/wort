...
def vectorize_bnc():
	...
	print('Word whitelist contains {} words!'.format(len(whitelist)))
	import math
	for log_sppmi, sppmi in zip([0], [0]):#zip([0, math.log(5), math.log(10)], [0, 5, 10]):
		for pmi_type in ['ppmi']:
			for cds in [1.]:
				for window_size in [7]:#[3, 5, 10, 7]:
					for weighting_fn in ['gaussian']:#['constant', 'aggressive', 'very_aggressive', 'harmonic', 'distance', 'inverse_harmonic', 'gaussian']:
...
def vectorize_wikipedia():
	...
	#out_path = os.path.join(paths.get_dataset_path(), 'wikipedia', 'wort_vectors')
	out_path = os.path.join('/bkp/thk22/_datasets/wikipedia/', 'wikipedia', 'wort_vectors')
	if (not os.path.exists(out_path)):
		os.makedirs(out_path)
			...
			for cds in [1., 0.75]:
				for window_size in [2, 1, 5]:# [5, 2]:
					for dim in [50, 100, 300]:
						...
						transformed_out_path = os.path.join('/bkp/thk22/_datasets/wikipedia/', 'wikipedia', 'wort_vectors', 'wort_model_ppmi_lemma-True_window-{}_cds-{}-sppmi_shift-{}_dim={}'.format(
