...
class ConfigRegistry(object):
	def __init__(self, path, min_frequency, lowercase, stop_words, encoding, max_features, preprocessor, tokenizer,
				 ...
				 wort_white_list, window_size, context_window_weighting, binary, weighting, cds, sppmi_shift, max_frequency):
		...
		self.max_frequency_ = max_frequency
	...
	def vocab_cache_folder(self):
		...
		cursor.execute(stmt, (self.min_frequency_, self.max_frequency_, 1 if self.lowercase_ else 0, str(self.stop_words_),
							  self.encoding_, -1 if self.max_features_ is None else self.max_features_, str(self.preprocessor_),
	...
	def register_vocab(self):
		...
		stmt = """
			INSERT INTO Vocab (min_frequency, max_frequency, lowercase, stop_words, encoding, max_features, preprocessor, 
							  tokenizer, analyzer, token_pattern, decode_error, strip_accents, input, ngram_range, 
							  random_state, subsampling_rate, wort_white_list, sub_folder)
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
		"""
	...
	def register_cooccurrence_matrix(self):
		...
		stmt = """
			INSERT INTO Cooccurrence_Matrix (min_frequency, max_frequency, lowercase, stop_words, encoding, max_features, 
											preprocessor, tokenizer, analyzer, token_pattern, decode_error, strip_accents, 
											input, ngram_range, random_state, subsampling_rate, wort_white_list, window_size, 
											context_window_weighting, binary, sub_folder)
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
		"""

		sub_folder = str(uuid.uuid1())
		cursor.execute(stmt, (self.min_frequency_, self.max_frequency_, 1 if self.lowercase_ else 0,
							  str(self.stop_words_), self.encoding_, -1 if self.max_features_ is None else self.max_features_,
							  str(self.preprocessor_), str(self.tokenizer_), str(self.analyzer_), self.token_pattern_,
							  self.decode_error_, str(self.strip_accents_),  self.input_, str(self.ngram_range_),
							  str(self.random_state_), 0.0 if self.subsampling_rate_ is None else self.subsampling_rate_,
							  str(self.wort_white_list_), str(self.window_size_), self.context_window_weighting_,
	...
	def register_pmi_matrix(self):
		...
		stmt = """
			INSERT INTO PMI_Matrix (min_frequency, max_frequency, lowercase, stop_words, encoding, max_features, preprocessor, tokenizer,
