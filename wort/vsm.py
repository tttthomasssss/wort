...
class VSMVectorizer(BaseEstimator, VectorizerMixin):
	...
	def _construct_cooccurrence_matrix(self, raw_documents):
		...
		self.M_ = sparse.coo_matrix((data, (rows, cols)), dtype=np.uint64) # Scipy seems to not handle numeric overflow in a very graceful manner
	...
	def get_vector(self, entry):
		self.T_[self.inverted_index_[entry]]
